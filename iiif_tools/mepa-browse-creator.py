...
with open('MEPA Bridge Export.txt', 'r') as f:
    ...
    next(r)
    for row in r:
        creator = row[1].split(' (')[0]
        title = mepa_title(row[3])
        if creator:
            creator_filename = 'mepa-browse-creator-' + safe(creator) + '.json'
            location = row[12].split(';')[0].replace(' [creation]', '').split(', ')
            url = mepa_url(row[12].split(';')[1].split(',')[4].split(' ')[6])
    
            if not creator_filename in creators:
                creators[creator_filename] = set()
            creators[creator_filename].add(url)
...
for creator_filename, members in creators.items():
    collection = {
        "label": creator,
        "@id": "https://iiif-collection.lib.uchicago.edu/mepa/" + creator_filename,
    ...
    with open(creator_filename, 'w') as f:
